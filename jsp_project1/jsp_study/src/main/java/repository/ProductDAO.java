...
public class ProductDAO implements DAO {
	...
	/* insert 영역 */
	public int insert(ProductVO ins) {
		System.out.println(">>> DAO 접근 완료.");
		qurey="insert into product(pname,price,madeby) values(?,?,?)";
		try {
			pst = conn.prepareStatement(qurey);
			pst.setString(1, ins.getPname());
			pst.setInt(2, ins.getPrice());
			pst.setString(3, ins.getMadeby());
	...
	public int edit(ProductVO pno) {
		...
		try {
			pst = conn.prepareStatement(qurey);
			pst.setString(1, pno.getPname());
			pst.setInt(2, pno.getPrice());
			pst.setString(3, pno.getMadeby());
			pst.setInt(4, pno.getPno());
